...
> **Status, 2026-08-26.** Authored 2026-05-31 on the `HDCA_Neurons` branch and
> carried over here because its problem statement and its critiques are still the
> best account of this area. Most of it has now shipped on
> `feature/subatlas-consistency`, though not in the shape drafted below:
>
> - **Improvement A (seed injection)** — shipped, but driven by a judged
>   `primacy` call in `subatlas_consistency.json` rather than by a raw cells-share
>   threshold. `citation_traverse_input` takes a `seeds` array; the defining paper
>   is priority 0.
> - **Improvement B (`annotation_sources.json`)** — shipped as two objects rather
>   than one, and the split matters. `subatlas_contributors.json` is the
>   deterministic, cutoff-applied view (no LLM); `subatlas_consistency.json` is the
>   judgement over it. The draft's single classification vocabulary (`inherited` /
>   `relabelled` / `merged` / `de_novo` / `external_taxonomy`) was replaced by a
>   SKOS relation (exact / broad / narrow / related / no match) plus an explicit
>   `primacy` call, because "how did the atlas treat this label" and "do these two
>   labels mean the same thing" are different questions and the draft's vocabulary
>   answered only the first.
> - **Improvement C (fail fast on missing primary source)** — shipped as
>   `report_checker.check_defining_paper`.
> - **Improvement D (full text for provenance sections)** — partly: the
>   subagent reads the contributing paper's own text via
>   `cli_annotate fetch --local`, but there is still no `read_section` helper.
> - **Pipeline bug 1** (agentic route ignores the local index) — fixed by
>   `cli_annotate fetch --local`.
> - **Pipeline bugs 2 and 3** (`traverse_local` catalogues only the top-ranked
>   paper; `local_<hash>` ids don't round-trip through S2 backfill) — **still
>   open**, in the deprecated graph.
> - **Bug 4** (no atlas full-text channel for the orchestrator) — still open.
>
> The "Threshold sensitivity" and "Atlas Methods text isn't always explicit"
> critiques below both stand, and the cutoff calibration in
> `services/subatlas_contributors.py` is the answer to the first.
>
> Read below as the original reasoning, not as outstanding work.
